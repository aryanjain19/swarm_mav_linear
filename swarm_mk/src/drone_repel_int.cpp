...
std::vector<float> app_vel_vec(std::vector<float>vec_del_pos,std::vector<float>vec_del_vel)
{
  // float vel1,vel2;
  // std::vector<float> vec_del_pos {vec_pos1[0]-vec_outdrone_pos[0] , vec_pos1[1]-vec_outdrone_pos[1] , vec_pos1[2]-vec_outdrone_pos[2]};
  // std::vector<float> vec_del_vel {vec_vel1[0]-vec_outdrone_vel[0],vec_vel1[1]-vec_outdrone_vel[1],vec_vel1[2]-vec_outdrone_vel[2]};
  float mod_del_pos = (pow(pow(vec_del_pos[0],2)+pow(vec_del_pos[1],2)+pow(vec_del_pos[2],2),0.5));
  float mod_del_vel = (pow(pow(vec_del_vel[0],2)+pow(vec_del_vel[1],2)+pow(vec_del_vel[2],2),0.5));

  if(mod_del_pos<0.0001 && mod_del_vel<0.0001)
  {
    mod_del_pos=0.001;mod_del_vel=0.001;
  }

  float dot_prod = std::inner_product(vec_del_pos.begin(),vec_del_pos.end(),vec_del_vel.begin(),0.0);

  float cos_theta = dot_prod/(mod_del_pos*mod_del_vel);

  for(int i =0;i<3;i++)
  {
    vec_del_vel[i] = vec_del_vel[i]*cos_theta;
  }

  return(vec_del_vel); // if positive then going away // if negative then coming closer.
}
  ...
  if (r12<(min_dist_vel+arm_length) || r13<(min_dist_vel+arm_length) || r14<(min_dist_vel+arm_length) || r23<(min_dist_vel+arm_length) || r24<(min_dist_vel+arm_length) || r34<(min_dist_vel+arm_length))
    {
     ...
     // nearest drone position for drone 4
      ...
      std::vector<float> del_vec_pos1 {del_rx1,del_ry1,del_rz1};
      std::vector<float> del_vec_vel1 {del_vx1,del_vy1,del_vz1};
      std::vector<float> app_vel1 = app_vel_vec(del_vec_pos1,del_vec_vel1);
      float mod_app_vel1 = (pow(pow(app_vel1[0],2)+pow(app_vel1[1],2)+pow(app_vel1[2],2),0.5));
      ...
      std::vector<float> del_vec_pos2 {del_rx2,del_ry2,del_rz2};
      std::vector<float> del_vec_vel2 {del_vx2,del_vy2,del_vz2};
      std::vector<float> app_vel2 = app_vel_vec(del_vec_pos2,del_vec_vel2);
      float mod_app_vel2 = (pow(pow(app_vel2[0],2)+pow(app_vel2[1],2)+pow(app_vel2[2],2),0.5));
      ...
      std::vector<float> del_vec_pos3 {del_rx3,del_ry3,del_rz3};
      std::vector<float> del_vec_vel3 {del_vx3,del_vy3,del_vz3};
      std::vector<float> app_vel3 = app_vel_vec(del_vec_pos3,del_vec_vel3);
      float mod_app_vel3 = (pow(pow(app_vel3[0],2)+pow(app_vel3[1],2)+pow(app_vel3[2],2),0.5));
      ...
      std::vector<float> del_vec_pos4 {del_rx4,del_ry4,del_rz4};
      std::vector<float> del_vec_vel4 {del_vx4,del_vy4,del_vz4};
      std::vector<float> app_vel4 = app_vel_vec(del_vec_pos4,del_vec_vel4);
      float mod_app_vel4 = (pow(pow(app_vel4[0],2)+pow(app_vel4[1],2)+pow(app_vel4[2],2),0.5));
    ...
    // Condition for inner region ---------------------------------------------------------------------------------
      if (r12<min_dist+arm_length || r13<min_dist+arm_length || r14<min_dist+arm_length || r23<min_dist+arm_length || r24<min_dist+arm_length || r34<min_dist+arm_length)
      {
        ...
        // tar_y1 = y_pos1 + repel_const*(del_ry1/pow(mod_r1,exp_r));
        // tar_z1 = z_pos1 + repel_const*(del_rz1/pow(mod_r1,exp_r));        
        // tar_x1 = x_pos1 + vel_repel_const*(del_vx1*del_rx1/pow(mod_r1,exp_r));
        // tar_y1 = y_pos1 + vel_repel_const*(del_vy1*del_ry1/pow(mod_r1,exp_r));
        // tar_z1 = z_pos1 + vel_repel_const*(del_vz1*del_rz1/pow(mod_r1,exp_r));


        for(int a=0;a<3;a++)
        {if(app_vel1[a] > 0)app_vel1[a]=0;}

        for(int a=0;a<3;a++)
        {if(app_vel2[a] > 0)app_vel2[a]=0;}

        for(int a=0;a<3;a++)
        {if(app_vel3[a] > 0)app_vel3[a]=0;}

        for(int a=0;a<3;a++)
        {if(app_vel4[a] > 0)app_vel4[a]=0;}
        


        float d1 = -vel_repel_const * mod_app_vel1/pow(mod_r1,3);
        float d2 = -vel_repel_const * mod_app_vel2/pow(mod_r2,3);             //d1 - distance between the target place of that particular drone and its initial position
        float d3 = -vel_repel_const * mod_app_vel3/pow(mod_r3,3);
        float d4 = -vel_repel_const * mod_app_vel4/pow(mod_r4,3);

        tar_x1 = x_pos1 + d1*del_rx1*app_vel1[0]/mod_r1 + repel_const*(del_rx1/pow(mod_r1,exp_r));
        tar_y1 = y_pos1 + d1*del_ry1*app_vel1[1]/mod_r1 + repel_const*(del_ry1/pow(mod_r1,exp_r));
        tar_z1 = z_pos1 + d1*del_rz1*app_vel1[2]/mod_r1 + repel_const*(del_rz1/pow(mod_r1,exp_r)) + ((((double)rand()/RAND_MAX)*6)-3);
        ...
        // tar_y2 = y_pos2 + repel_const*(del_ry2/pow(mod_r2,exp_r));
        // tar_z2 = z_pos2 + repel_const*(del_rz2/pow(mod_r2,exp_r));   
        // tar_x2 = x_pos2 + vel_repel_const*(del_vx2*del_rx2/pow(mod_r2,exp_r));
        // tar_y2 = y_pos2 + vel_repel_const*(del_vy2*del_ry2/pow(mod_r2,exp_r));
        // tar_z2 = z_pos2 + vel_repel_const*(del_vz2*del_rz2/pow(mod_r2,exp_r));

        // tar_x2 = x_pos2 - vel_repel_const*(del_vx2/pow(mod_r2,exp_r));
        // tar_y2 = y_pos2 - vel_repel_const*(del_vy2/pow(mod_r2,exp_r));
        // tar_z2 = z_pos2 - vel_repel_const*(del_vz2/pow(mod_r2,exp_r));
        tar_x2 = x_pos2 + d2*del_rx2*app_vel2[0]/mod_r2 + repel_const*(del_rx2/pow(mod_r2,exp_r));
        tar_y2 = y_pos2 + d2*del_ry2*app_vel2[1]/mod_r2 + repel_const*(del_ry2/pow(mod_r2,exp_r));
        tar_z2 = z_pos2 + d2*del_rz2*app_vel2[2]/mod_r2 + repel_const*(del_rz2/pow(mod_r2,exp_r)) + ((((double)rand()/RAND_MAX)*6)-3);
        // ROS_INFO(" random  : [%f].", ((((double)rand()/RAND_MAX)*4)-2));
        ...
        // tar_y3 = y_pos3 + repel_const*(del_ry3/pow(mod_r3,exp_r));
        // tar_z3 = z_pos3 + repel_const*(del_rz3/pow(mod_r3,exp_r)); 
        // tar_x3 = x_pos3 + vel_repel_const*(del_vx3*del_rx3/pow(mod_r3,exp_r));
        // tar_y3 = y_pos3 + vel_repel_const*(del_vy3*del_ry3/pow(mod_r3,exp_r));
        // tar_z3 = z_pos3 + vel_repel_const*(del_vz3*del_rz3/pow(mod_r3,exp_r)); 

        // tar_x3 = x_pos3 - vel_repel_const*(del_vx3/pow(mod_r3,exp_r));
        // tar_y3 = y_pos3 - vel_repel_const*(del_vy3/pow(mod_r3,exp_r));
        // tar_z3 = z_pos3 - vel_repel_const*(del_vz3/pow(mod_r3,exp_r));       
        tar_x3 = x_pos3 + d3*del_rx3*app_vel3[0]/mod_r3 + repel_const*(del_rx3/pow(mod_r3,exp_r));
        tar_y3 = y_pos3 + d3*del_ry3*app_vel3[1]/mod_r3 + repel_const*(del_ry3/pow(mod_r3,exp_r));
        tar_z3 = z_pos3 + d3*del_rz3*app_vel3[2]/mod_r3 + repel_const*(del_rz3/pow(mod_r3,exp_r)) + ((((double)rand()/RAND_MAX)*6)-3);
        ...
        // tar_y4 = y_pos4 + repel_const*(del_ry4/pow(mod_r4,exp_r));
        // tar_z4 = z_pos4 + repel_const*(del_rz4/pow(mod_r4,exp_r));
        // tar_x4 = x_pos4 + vel_repel_const*(del_vx4*del_rx4/pow(mod_r4,exp_r));
        // tar_y4 = y_pos4 + vel_repel_const*(del_vy4*del_ry4/pow(mod_r4,exp_r));
        // tar_z4 = z_pos4 + vel_repel_const*(del_vz4*del_rz4/pow(mod_r4,exp_r)); 

        // tar_x4 = x_pos4 - vel_repel_const*(del_vx4/pow(mod_r4,exp_r));
        // tar_y4 = y_pos4 - vel_repel_const*(del_vy4/pow(mod_r4,exp_r));
        // tar_z4 = z_pos4 - vel_repel_const*(del_vz4/pow(mod_r4,exp_r));
        tar_x4 = x_pos4 + d4*del_rx4*app_vel4[0]/mod_r4 + repel_const*(del_rx4/pow(mod_r4,exp_r));
        tar_y4 = y_pos4 + d4*del_ry4*app_vel4[1]/mod_r4 + repel_const*(del_ry4/pow(mod_r4,exp_r));
        tar_z4 = z_pos4 + d4*del_rz4*app_vel4[2]/mod_r4 + repel_const*(del_rz4/pow(mod_r4,exp_r)) + ((((double)rand()/RAND_MAX)*6)-3);        
        ...
        {
          ros::spinOnce();
          looprate.sleep(); // 1/10= 0.1 sec of wait
        }
      ...
      if(key==0) // bigger loop
      {
        ...
        float d1 = -vel_repel_const * mod_app_vel1/pow(mod_r1,2);
        float d2 = -vel_repel_const * mod_app_vel2/pow(mod_r2,2);             //d1 - distance between the target place of that particular drone and its initial position
        float d3 = -vel_repel_const * mod_app_vel3/pow(mod_r3,2);
        float d4 = -vel_repel_const * mod_app_vel4/pow(mod_r4,2);

        float corr=0;
        corr = d1*del_rx1*app_vel1[0]/mod_r1;
        if(abs(corr)<0.01)tar_x1 = x_coord+radius;
        else tar_x1 = x_pos1 + corr;

        corr = d1*del_ry1*app_vel1[1]/mod_r1;
        if(abs(corr)<0.01)tar_y1 = y_coord+radius;
        else tar_y1 = y_pos1 + corr;

        corr = d1*del_rz1*app_vel1[2]/mod_r1;
        if(abs(corr)<0.01)tar_z1 = z_coord;
        else tar_z1 = z_pos1 + corr;

        // tar_x1 = x_pos1 + d1*del_rx1*app_vel1[0]/mod_r1;
        // tar_y1 = y_pos1 + d1*del_ry1*app_vel1[1]/mod_r1;
        // tar_z1 = z_pos1 + d1*del_rz1*app_vel1[2]/mod_r1;

        // ROS_INFO(" random 2 _1: [%f].",d1*del_rx1*app_vel1[0]/mod_r1);
        // ROS_INFO(" random 2 _1: [%f].",d1*del_ry1*app_vel1[1]/mod_r1);
        // ROS_INFO(" random 2 _1: [%f].",d1*del_rz1*app_vel1[2]/mod_r1);
        ...
        // tar_y2 = y_pos2 - vel_repel_const*(del_vy2/pow(mod_r2,exp_r));
        // tar_z2 = z_pos2 - vel_repel_const*(del_vz2/pow(mod_r2,exp_r));
        // tar_x2 = x_pos2 + d2*del_rx2*app_vel2[0]/mod_r2;
        // tar_y2 = y_pos2 + d2*del_ry2*app_vel2[1]/mod_r2;
        // tar_z2 = z_pos2 + d2*del_rz2*app_vel2[2]/mod_r2;

        corr = d2*del_rx2*app_vel2[0]/mod_r2;
        if(abs(corr)<0.01)tar_x2 = x_coord-radius;
        else tar_x2 = x_pos2 + corr;

        corr = d2*del_ry2*app_vel2[1]/mod_r2;
        if(abs(corr)<0.01)tar_y2 = y_coord-radius;
        else tar_y2 = y_pos2 + corr;

        corr = d2*del_rz2*app_vel2[2]/mod_r2;
        if(abs(corr)<0.01)tar_z2 = z_coord;
        else tar_z2 = z_pos2 + corr;
        ...
        // tar_y3 = y_pos3 - vel_repel_const*(del_vy3/pow(mod_r3,exp_r));
        // tar_z3 = z_pos3 - vel_repel_const*(del_vz3/pow(mod_r3,exp_r));       
        // tar_x3 = x_pos3 + d3*del_rx3*app_vel3[0]/mod_r3;
        // tar_y3 = y_pos3 + d3*del_ry3*app_vel3[1]/mod_r3;
        // tar_z3 = z_pos3 + d3*del_rz3*app_vel3[2]/mod_r3;
        // ROS_INFO(" random 2 _3: [%f].",d3*del_rx3*app_vel3[0]/mod_r3);
        // ROS_INFO(" random 2 _3: [%f].",d3*del_ry3*app_vel3[1]/mod_r3);
        // ROS_INFO(" random 2 _3: [%f].",d3*del_rz3*app_vel3[2]/mod_r3);

        corr = d3*del_rx3*app_vel3[0]/mod_r3;
        if(abs(corr)<0.01)tar_x3 = x_coord-radius;
        else tar_x3 = x_pos3 + corr;

        corr = d3*del_ry3*app_vel3[1]/mod_r3;
        if(abs(corr)<0.01)tar_y3 = y_coord+radius;
        else tar_y3 = y_pos3 + corr;

        corr = d3*del_rz3*app_vel3[2]/mod_r3;
        if(abs(corr)<0.01)tar_z3 = z_coord;
        else tar_z3 = z_pos3 + corr;
        ...
        // tar_y4 = y_pos4 - vel_repel_const*(del_vy4/pow(mod_r4,exp_r));
        // tar_z4 = z_pos4 - vel_repel_const*(del_vz4/pow(mod_r4,exp_r));
        // tar_x4 = x_pos4 + d4*del_rx4*app_vel4[0]/mod_r4;
        // tar_y4 = y_pos4 + d4*del_ry4*app_vel4[1]/mod_r4;
        // tar_z4 = z_pos4 + d4*del_rz4*app_vel4[2]/mod_r4;

        corr = d4*del_rx4*app_vel4[0]/mod_r4;
        if(abs(corr)<0.01)tar_x4 = x_coord+radius;
        else tar_x4 = x_pos4 + corr;

        corr = d4*del_ry4*app_vel4[1]/mod_r4;
        if(abs(corr)<0.01)tar_y4 = y_coord-radius;
        else tar_y4 = y_pos4 + corr;

        corr = d4*del_rz4*app_vel4[2]/mod_r4;
        if(abs(corr)<0.01)tar_z4 = z_coord;
        else tar_z4 = z_pos4 + corr;
