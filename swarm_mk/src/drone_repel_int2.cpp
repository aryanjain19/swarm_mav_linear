  ...
  if (r12<(min_dist_vel+arm_length) || r13<(min_dist_vel+arm_length) || r14<(min_dist_vel+arm_length) || r23<(min_dist_vel+arm_length) || r24<(min_dist_vel+arm_length) || r34<(min_dist_vel+arm_length))
    ...
    // Condition for inner region ---------------------------------------------------------------------------------
      ...
      if(key==2) // bigger loop
      {
        ...
        float d1 = -vel_repel_const * mod_app_vel1/pow(mod_r1,1);
        float d2 = -vel_repel_const * mod_app_vel2/pow(mod_r2,1);             //d1 - distance between the target place of that particular drone and its initial position
        float d3 = -vel_repel_const * mod_app_vel3/pow(mod_r3,1);
        float d4 = -vel_repel_const * mod_app_vel4/pow(mod_r4,1);
